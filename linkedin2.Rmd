...
datatable(linkedin2, options=list(pageLength=5))
```
...
In latter segment, I explored how beauty could be predicted using average position length, average previous tenure, and tenure length. From the statistics point of view, it stated that all three of the independent variables were significant (<0.5 p-value), but when we took a deeper look into the R^2^, we discovered that only 2.7% of the data points were explained by the independent variables. 
