...
Operationalize: How to make your research actually go into affect? 
Intervention: After operationaliziation, can you notice any change happening? What datapoints would be taken to measure the success? 
  - Cost/benefit ratio? 
...
include("plyr")
include("dplyr")
suppressMessages(purl("linkedin2.Rmd", output = "part2.r"))
source("part2.r")
...
```{r}
#calculate number of positions and calculate avg pay. 
...
# Analysis
What I want to explore is if I could be able to predict how a long a person is going to stay at their positions. Let's begin the model. 
...
After improving our model, immediately, I notice that our the number of linkedin connections count is no longer significant. I will go ahead and remove that independent variable. 
...
#New train models with all the significant variables from pervious model and with our new director boolean variable. 
train_model <- lm(avgMemberPosDuration ~ ageEstimate + avg_pay + director_bool, data = train)
...
Now that the number of linkedin connections is removed from the independent variables, we could see our final result. All of our variables are now significant with an overall R^2 of 15%. This means that using the age, average pay of director position, and for directors, we can estimate about 15% of the time of how long they will stay at their job. If we look deeper into 